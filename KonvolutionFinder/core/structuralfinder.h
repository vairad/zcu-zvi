...
class StructuralFinder : public QThread {
    ...
    InclusionDescriptor *descriptor;
    ...
    bool checkMinVertices(unsigned int vertices);
    bool checkMaxVertices(unsigned int vertices);
    bool checkExtent(double contourArea, double rectArea);
public:
    static int defaultThreshold;

    StructuralFinder(FilenameFactory *names, InclusionDescriptor *descriptor, int threshold);
signals:
